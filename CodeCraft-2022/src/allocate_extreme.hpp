...
private:
    ...
    {
        bool operator()(
            const tuple<i32, i32, i32> &x,
            const tuple<i32, i32, i32> &y)
        {
    ...
    std::priority_queue<
        tuple<i32, i32, i32>, vector<tuple<i32, i32, i32>>, CompareFunc>
        extreme_que;
...
private:
    void initQueue()
    {
        for (i32 time = 0; time < requests.size(); time++)
        {
            ...
            {
                extreme_que.emplace(make_tuple(time, server, requests[time][server]));
            }
        }
    }
    ...
    void popQueue()
    {
        ...
        {
            if (total_select >= g.n_server * g.n_time * peak_ratio)
                break;
            ...
            if (ratio_flag[server] >= g.n_time * peak_ratio)
                continue;
            ratio_flag[server]++;
            total_select++;
            // compute alocation
